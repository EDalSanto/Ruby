...
class Array
  def my_uniq
    uniqs = []
    self.each { |num| uniqs << num if !uniqs.include?(num) }
    uniqs.sort
  end
end
...
class Array
  def two_sum
    ...
    self.each_index do |left_idx|
      (left_idx+1..last_idx).each do |right_idx|
        pair_sum = self[left_idx] + self[right_idx]
        zero_pairs << [left_idx,right_idx].sort if pair_sum == 0
      end
    end
    zero_pairs
  end
...
class Array
  def median
    ...
    sorted.length.odd? ? sorted[middle] : (sorted[middle] + sorted[middle-1])/2.0
  end
end
...
class Array
  ...
  def my_transpose
    transposed = []
    row_length = self[0].length # Works with any rectangular matrix!
