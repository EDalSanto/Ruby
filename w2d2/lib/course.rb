...
class Course
  ...
  def conflicts_with?(other_course)
    # for two courses to conflict, they must be on the same day and time block
    intersecting_days = @days & other_course.days
    return true if @time_block == other_course.time_block && !intersecting_days.empty?
    false
  end
